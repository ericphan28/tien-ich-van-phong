...
function Get-AllModules {
    ...
    if (Test-Path $modulesPath) {
        ...
        foreach ($module in $modules) {
            $manifestPath = Join-Path $modulesPath $module "manifest.json"            if (Test-Path $manifestPath) {
...
try {
    ...
    switch ($Command.ToLower()) {
        ...
        "package" {
            ...
            # Tạo file .zip từ folder
            $folderPath = Join-Path $OutputPath "$ModuleId-v*"
            $folders = Get-ChildItem -Path $folderPath -Directory 2>$null
            if ($folders) {
